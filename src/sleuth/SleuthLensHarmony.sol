...
contract SleuthLensHarmony {
    ...
    function buildCTokenAllData(CTokenInterface cToken) public returns (CTokenAllData memory) {
        ...
        if (compareStrings(cToken.symbol(), "cONE")) {
    ...
    function queryAllWithAccount(CTokenInterface[] calldata cTokens, address payable account) external returns (AccountAllData memory) {
        uint cTokenCount = cTokens.length;
